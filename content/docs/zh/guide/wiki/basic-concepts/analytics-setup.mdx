...
两种分析工具可以同时启用，互不冲突。需要持续性能分析时可额外配置 Pyroscope。
...
Pyroscope 是持续性能分析工具，用于采集 CPU/Mutex/Block 等剖析数据。

#### 1. 准备 Pyroscope 服务

确保 Pyroscope 服务可访问，并记录服务地址。
...
- `PYROSCOPE_APP_NAME`: 应用标识（建议区分环境/集群）
- `PYROSCOPE_BASIC_AUTH_USER` / `PYROSCOPE_BASIC_AUTH_PASSWORD`: 仅在启用 Basic Auth 时需要
- `PYROSCOPE_MUTEX_RATE` / `PYROSCOPE_BLOCK_RATE`: 采样率，值越小采样越频繁
- `HOSTNAME`: 实例标识（可选）
