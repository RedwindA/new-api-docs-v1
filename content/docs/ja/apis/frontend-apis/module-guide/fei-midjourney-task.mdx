...
APIプレフィックスは http(s)://`<your-domain>` に統一されています。
    ...
    画像生成タスクの管理システムです。タスクの状態追跡、進捗監視、結果表示などの機能をサポートしています。画像URL転送とバックグラウンドポーリング更新メカニズムを含みます。
...
- **パス**：`/api/mj/self`
- **認証要件**：ユーザー
- **機能概要**：現在のユーザーのMidjourneyタスクリストをページングで取得します。タスクIDと時間範囲によるフィルタリングをサポートします。
...
- 返却フィールド説明：
  - `id` （数値）: データベースレコードID
  - `mj_id` （文字列）: Midjourneyタスクの一意の識別子
  - `action` （文字列）: 操作タイプ、例：IMAGINE、UPSCALEなど
  - `prompt` （文字列）: 元のプロンプト
  - `prompt_en` （文字列）: 英語プロンプト
  - `status` （文字列）: タスクステータス midjourney.go：19
  - `progress` （文字列）: 完了進捗率
  - `image_url` （文字列）: 生成された画像URL
  - `video_url` （文字列）: 生成されたビデオURL
  - `video_urls` （文字列）: 複数のビデオURLのJSON配列文字列
...
- **パス**：`/api/mj/`
- **認証要件**：管理者
- **機能概要**：システム内の全てのMidjourneyタスクをページングで取得します。チャネルID、タスクID、時間範囲によるフィルタリングをサポートします。
...
- 返却フィールドはユーザー自身のタスクの全てのフィールドを含み、さらに以下が追加されます：
  - `user_id` （数値）: タスクの所有ユーザーID
