...
Call the OpenAI video generation API to generate videos, supporting models like Sora, and also supporting calling Kling, Jimeng, and Vidu using the OpenAI video format.
...
| input_reference | file   | No       | Input image file (used for image-to-video generation)             |
| metadata        | string | No       | Extended parameters (JSON string format)                          |

#### metadata Parameter Description

The `metadata` parameter is used to pass parameters specific to non-Sora models, such as image URLs, watermarks, and intelligent prompt rewriting for Ali Wan. The `metadata` parameter is in JSON string format, for example:
...
#### Ali Wan Video Generation Example
...
Query the status and results of a video generation task based on its task ID
...
Get detailed information about a video generation task based on its task ID
...
Directly returns the video file stream, with Content-Type `video/mp4`
...
- Ali Wan: Uses `wan2.5-t2v-preview` (text-to-video), `wan2.5-i2v-preview` (image-to-video), `wan2.2-i2v-flash`, `wan2.2-i2v-plus`, `wanx2.1-i2v-plus`, `wanx2.1-i2v-turbo`
- Kling AI: Uses `kling-v1`, `kling-v2-master`
- Jimeng: Uses `jimeng_vgfm_t2v_l20`, `jimeng_vgfm_i2v_l20`
- Vidu: Uses `viduq1`

## Ali Wan Special Notes

### Supported Features

- **Text-to-Video (t2v)**: Generates video using only text prompts
- **Image-to-Video (i2v)**: Generates video using text prompts + images
- **Keyframe-to-Video (kf2v)**: Generates video by specifying first and last frame images
...
- `prompt_extend`: Whether to enable intelligent prompt rewriting (default true)
- `audio`: Whether to add audio (only supported by wan2.5)
- `seed`: Random seed

### Model Features

- **wan2.5-i2v-preview**: Wanxiang 2.5 preview version, supports video with sound, recommended for use
...
1.  **Request Format**: Use `multipart/form-data` format, which is the official recommendation from OpenAI
2.  **`input_reference` Parameter**: Used for image-to-video functionality, use `@filename` syntax when uploading image files
...
5.  **Ali Wan Special Notes**:
    ...
    - For image-to-video, use `metadata.img_url` to pass the image URL
    - For keyframe-to-video, use `metadata.first_frame_url` and `metadata.last_frame_url`
6.  **Error Handling**: Implement appropriate retry mechanisms and error handling
7.  **Asynchronous Processing**: Video generation is an asynchronous task, requiring polling to query status
