...
✅ Success Response Example (without 2FA):
...
✅ Success Response Example (2FA required):
...
- `data` (object): User available group information mapping group.go: 25-48
  - `Key` (string): Group name
  - `ratio` (number/string): Group ratio, "自动" (auto) means automatically select optimal group
  - `desc` (string): Group description
